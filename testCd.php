...
<div class="frow bg-pic" style="flex: 1 0 auto;">
	...
	//	$result = $conn->query($sql);
		...
		$sql = "SELECT DISTINCT id,title,genre,publisher,producer,first_name,last_name,published_date,audience,language FROM Full_CD_View ORDER BY title LIMIT $page1,10;";
		...
		echo"<h3 align='center'><b>pages:</b><span>";
			$sql = "SELECT DISTINCT id,title FROM  Full_CD_View   ORDER BY title ;";
		$result = $conn->query($sql) or die("failed");
		 $count = $result->num_rows;
		$a =ceil($count/10);
		for($i =1;$i<=$a;$i++){
		    echo "<a id='page$i'href='http://www.databaseteam12.x10host.com/testCd.php?page=$i' ><b>$i</b></a>";
		}
		echo "</span></h3>";
		$conn->close();
...
<?php 
 function checkAvail($id,$conn){
    // echo "id is ".$id;
     $sql ="SELECT copy_num FROM Full_CD_View WHERE id= $id AND is_available ='available' LIMIT 1;";
