...
<div class="frow bg-pic" style="flex: 1 0 auto;">
	...
	<main class="grow-row fill-col">
		...
		$sql = "SELECT Media.id, Media.copy_num, Media.title, Author.first_name, 
		    Author.last_name, Media.published_date, Media.publisher, 
		    Media.is_available FROM Media LEFT OUTER JOIN Author 
		    on Media.id = Author.media_id  ORDER BY title;";
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				echo "<hr><h2>".$row["title"]."</h2>".$row2["first_name"]." "
				.$row2["last_name"]." ".$row["published_date"]."<br>"
				.$row["publisher"].".<br><br>
