...
<div class="frow bg-pic" style="flex: 1 0 auto;">
    ...
    <form name="form" action="" method="get">
		...
		if (isset($_GET['member-id'])) {
			...
			$sql_2 = "SELECT EXISTS (SELECT * FROM Faculty WHERE faculty_id = '$id');";
			$result_2 = $conn->query($sql_2);
				
			if ($result_1 && $result_2) {
				$sql_3 = "DELETE FROM Faculty WHERE faculty_id = '$id';";
				$result_3 = $conn->query($sql_3);
				$sql_4 = "UPDATE Member SET userAccount = 'Student' WHERE id = '$id';";
				...
				if (!($result_3) || !($result_4)) {
					...
					echo "<hr><div style=\"padding: 15px; background-color: #4CAF50; color: white;\">
						Faculty successfully removed.</span>";
				}
			}
			else if (!($result_1)) {
