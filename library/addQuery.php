...
function laptop($conn){
  ...
  if ($result->num_rows > 0) {
      ...
      $sql="INSERT INTO laptop (id,serial) VALUES ($val,'$serial')";
...
function addBook($conn){
 ...
 <?php
  function  addDigitalDVD_VHS($conn,$type){
         $id;
         if($type=="dvd"){$id="dvd_id";}else{ $id ="vhs_id";}
 ...
 <?php
  function   addDigitalCD_CASSETTE($conn,$type){
    $id;
    if($type=="cd"){$id="cd_id";}else{ $id ="cassette_id";}
